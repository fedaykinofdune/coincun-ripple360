...
if (!fs.existsSync('logs')) {
	...
	info('Created /logs directory'.red);
}
...
var meta = readMeta(function (meta) {
	...
	info('Current Ledger Index: '.cyan + curLedgerIdx);
	info('Last Ledger Index:    '.cyan + meta.ledger);
	info('Last Tx Hash:         '.cyan + meta.hash);
	info('Last Timesatmp:       '.cyan + meta.timestamp);
	...
	ws = new WebSocket(RIPPLE_SERVER);

	ws.on('open', function() {
		info('Connected to ' + 'RIPPLED'.blue + ' successfully...');
...
//		ws.send(JSON.stringify(msg));
	...
	ws.on('message', function(message) {
		...
		if (data.validated && data.account != RIPPLE_ADDRESS) {
			...
			info('Player Account:    ' + data.account);
			info('Bet Amount:        ' + parseInt(data.amount / RIPPLE_SINGLE));
			info('Our Bank Roll:     ' + parseInt(data.bankroll / RIPPLE_SINGLE) + ' XRP');

			debug('Ledger Index:     ' + data.ledger);
			debug('Transaction Hash: ' + data.hash);

			// do we have a destination tag
			if (data.tag)
				debug('Destination Tag:  ' + data.tag);
			...
			if (data.tag && data.tag == 777) {
				...
				info('Performing refill / donation to account...'.red);
			} else {
			...
			info('There are '.red + numTxs.toString() + 
				' transactions waiting to be processed.'.red);
			...
			if (meta.ledger != -1) {
				...
				for (var i = 0; i < numTxs; i++) {
					info('Player Account:    ' + data.txs[i].account);
					info('Bet Amount:        ' + parseInt(data.txs[i].amount / RIPPLE_SINGLE));
					info('Our Bank Roll:     ' + parseInt(data.txs[i].bankroll / RIPPLE_SINGLE) + ' XRP');

					debug('Ledger Index:     ' + data.txs[i].ledger);
					debug('Transaction Hash: ' + data.txs[i].hash);

					// do we have a destination tag
					// TODO: detect tag and perform specific actions
					// 		 e.g. 777 = refill transaction (DO NOT RUN GAMES)
					if (data.txs[i].tag)
						debug('Destination Tag:  ' + data.txs[i].tag);

					// perform specific actions based on Destination Tag
					// e.g. 777 = refill / donate transaction (DO NOT RUN GAMES)
					if (data.tag && data.tag == 777) {
						info('Performing refill / donation to account...'.blue);
					} else {
						// run the Coin Flip game
			...
			info('Requesting LIVE transactions...'.blue);
		} else if (data.result) {
			if (DEBUG)
				logSpacer();	// log spacer
...
function runCoinFlip(data) {
	info('Starting Coin Flip Game'.blue);
...
betLimit = 1000000000;
	...
	if (data.amount > betLimit) {
		info('WARNING!!!'.red + ' Someone\'s trying to bankrupt us -- don\'t let it happen');
	...
	info('Payout: '.blue + payout + ' (~' + parseInt(payout / RIPPLE_SINGLE) + ' XRP)');
...
function fairplayPoint(data, gameType) {
	info('Calculating Fairplay for '.blue + 
		gameType.toString() + '-bit' + 
		' Game...'.blue);
...
function writeMeta(data) {
	...
	fs.writeFile('meta.json', JSON.stringify(meta), function (err) {
		...
		info('meta.json'.blue + ' saved successfully...');
	});
